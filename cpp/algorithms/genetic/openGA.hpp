...
public:
	...
	void solve_init()
	{
		...
		{
			cerr<<"**************************************"<<endl;
			cerr<<"*             GA started             *"<<endl;
			cerr<<"**************************************"<<endl;
			cerr<<"population: "<<population<<endl;
			cerr<<"elite_count: "<<elite_count<<endl;
			cerr<<"crossover_fraction: "<<crossover_fraction<<endl;
		...
		if(!is_single_objective())
		{
			...
			if(!reference_vector_divisions)
			{
				...
				{
					cerr<<"**************************************"<<endl;
					cerr<<"reference_vector_divisions: "<<reference_vector_divisions<<endl;
					cerr<<"**************************************"<<endl;
				}
			}
		}
	...
	std::string stop_reason_to_string(StopReason stop)
	{
		switch(stop)
		{
			case StopReason::ReachedZeroCost:
				return "reached zero cost";
				break;
...
protected:
	...
	void show_stop_reason(StopReason stop)
	{
		if(verbose or verbose2)
		{
			...
			if(stop==StopReason::Undefined)
				cerr<<"There is a bug in this function";
			else
				cerr<<stop_reason_to_string(stop);
			cerr<<endl;
			cerr<<"**************************************"<<endl;
		}
	}
	...
	void generate_selection_chance_on_cost(thisGenerationType &gen, const vector<int> &rank)
	{
		double chance_cumulative=0.0;
		unsigned int N=(unsigned int)gen.chromosomes.size();
		gen.selection_chance_cumulative.clear();
		for(unsigned int i=0;i<N;i++) {
			chance_cumulative += 1.0/sqrt(double(gen.chromosomes[i].total_cost+1));
			gen.selection_chance_cumulative.push_back(chance_cumulative);
		}
		for(unsigned int i=0;i<N;i++)
			gen.selection_chance_cumulative[i]=gen.selection_chance_cumulative[i]/gen.selection_chance_cumulative[population-1];
	...
	void init_population_range(thisGenerationType *p_generation0, int index_from, int index_to, unsigned int *attemps,
	...
	****************************************************/
	void init_population(thisGenerationType &generation0)
	{
		...
		if(verbose)
		{
			cerr<<"Initial population of "<<population<<" was created with "<<total_attempts<<" attemps."<<endl;
		}
	}
	...
	int select_parent(const thisGenerationType &g)
	{
		...
		// cerr << "select parent: " << endl;
		// for(int i = 0; i < N_max; i++) {
		// 	cerr << g.selection_chance_cumulative[i] << ' ';
		// 	if(g.selection_chance_cumulative[i] >= 1 and g.selection_chance_cumulative[i-1] < 1) 
		// 		cerr << i << ' ';
		// }
		double r=random01();
		// cerr << r << endl;
		...
		for(int index=x_index_begin;index<=x_index_end;index++)
		{
			...
			while(!successful)
			{
				...
				if(pidx_c1==pidx_c2)
					continue ;
				if(verbose)
					cerr<<"Crossover of chromosomes "<<pidx_c1<<","<<pidx_c2<<endl;
				...
				{
					if(verbose)
						cerr<<"Mutation of chromosome "<<endl;
	...
	void crossover_and_mutation(thisGenerationType &new_generation)
	{
		...
		if(verbose)
		{
			cerr<<"Mutations and crossovers of "<<N_add<<" solutions are calculated with "<<total_attempts<<" attemps."<<endl;
		}
	}
	...
	StopReason stop_critera()
	{
		...
		if(is_single_objective())
		{
			...
			if(verbose2 and generation_step % 20 == 0)
				cerr << "Generation " << generation_step << "'s best answer: " << g2.best_total_cost << endl;
			if(g2.best_total_cost == 0)
				return StopReason::ReachedZeroCost;
		}
